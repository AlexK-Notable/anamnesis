...
class TestTier2EligibilityThresholds:
    """Tests for Tier 2 response TOON eligibility based on array size."""

    @pytest.fixture
    def encoder(self):
        """Create a ToonEncoder with default min_array_size=5."""
        from anamnesis.utils.toon_encoder import ToonEncoder
        return ToonEncoder(min_array_size=5)

    @pytest.fixture
    def encoder_threshold_10(self):
        """Create a ToonEncoder with min_array_size=10."""
        from anamnesis.utils.toon_encoder import ToonEncoder
        return ToonEncoder(min_array_size=10)

    def test_tier2_eligible_when_array_meets_threshold(self, encoder):
        """Tier 2 response is eligible when array size >= min_array_size."""
        data = {
            "concepts": [{"id": i} for i in range(5)],  # Exactly at threshold
            "patterns": [],
        }
        assert encoder.is_toon_eligible("AnalyzeCodebaseResponse", data) is True

    def test_tier2_not_eligible_when_array_below_threshold(self, encoder):
        """Tier 2 response is NOT eligible when array size < min_array_size."""
        data = {
            "concepts": [{"id": i} for i in range(4)],  # Below threshold
            "patterns": [],
        }
        assert encoder.is_toon_eligible("AnalyzeCodebaseResponse", data) is False

    def test_tier2_eligible_when_any_tabular_field_meets_threshold(self, encoder):
        """Tier 2 is eligible if ANY tabular field meets threshold."""
        data = {
            "concepts": [{"id": i} for i in range(3)],  # Below threshold
            "patterns": [{"name": f"p{i}"} for i in range(10)],  # Above threshold
        }
        assert encoder.is_toon_eligible("AnalyzeCodebaseResponse", data) is True

    def test_tier2_not_eligible_when_all_arrays_below_threshold(self, encoder):
        """Tier 2 is NOT eligible when all tabular fields are below threshold."""
        data = {
            "concepts": [{"id": i} for i in range(2)],
            "patterns": [{"name": "p1"}],
        }
        assert encoder.is_toon_eligible("AnalyzeCodebaseResponse", data) is False

    def test_tier2_eligible_with_custom_threshold(self, encoder_threshold_10):
        """Custom min_array_size threshold is respected."""
        data = {
            "concepts": [{"id": i} for i in range(8)],  # Below 10
            "patterns": [],
        }
        assert encoder_threshold_10.is_toon_eligible("AnalyzeCodebaseResponse", data) is False

        data_large = {
            "concepts": [{"id": i} for i in range(15)],  # Above 10
            "patterns": [],
        }
        assert encoder_threshold_10.is_toon_eligible("AnalyzeCodebaseResponse", data_large) is True

    def test_tier2_defaults_to_eligible_without_data(self, encoder):
        """Tier 2 response defaults to eligible when no data provided."""
        # Without data to check, we can't verify array sizes
        assert encoder.is_toon_eligible("AnalyzeCodebaseResponse") is True

    def test_tier2_not_eligible_with_nested_arrays(self, encoder):
        """Tier 2 response is NOT eligible if data contains nested arrays."""
        data = {
            "concepts": [
        ...
        # Even though concepts has 2 items, nested arrays disqualify it
        assert encoder.is_toon_eligible("AnalyzeCodebaseResponse", data) is False

    def test_tier1_ignores_threshold(self, encoder):
        """Tier 1 responses are always eligible (threshold doesn't apply)."""
        data = {
            "results": [{"id": 1}],  # Only 1 item, below threshold
        }
        # SearchCodebaseResponse is Tier 1, so threshold doesn't apply
        assert encoder.is_toon_eligible("SearchCodebaseResponse", data) is True

    def test_tier3_always_ineligible(self, encoder):
        """Tier 3 responses are never eligible regardless of array size."""
        data = {
            "insights": [{"id": i} for i in range(100)],  # Huge array
        }
        # SemanticInsightsResponse is Tier 3 (has nested relationships[])
        assert encoder.is_toon_eligible("SemanticInsightsResponse", data) is False

    def test_unknown_response_type_is_tier3(self, encoder):
        """Unknown response types default to Tier 3 (not eligible)."""
        data = {"items": [{"id": i} for i in range(100)]}
        assert encoder.is_toon_eligible("UnknownResponseType", data) is False
        assert encoder.get_tier("UnknownResponseType") == 3
